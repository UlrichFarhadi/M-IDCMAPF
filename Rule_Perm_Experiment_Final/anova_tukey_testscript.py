...
def find_best_rule_perm(filename, num_perms=720, num_repetitions=100):
    #filename = "random400_100runs_48_48_20.csv"
    data = read_my_file(filename)
    cost_averages = []
    span_averages = []
    rule_perm = []
    for n in range(num_perms): #len(data) ????
        sum_cost = 0
        sum_span = 0
        fails = 0 
        for i in range(n*num_repetitions, (n+1)*num_repetitions):
            # if int(data[i][2]) == 1000:
            #     fails += 1
            sum_cost += int(data[i][1])
            sum_span += int(data[i][2])

        rule_perm.append(data[n*num_repetitions][0])
        cost_averages.append(sum_cost/(num_repetitions-fails))
        span_averages.append(sum_span/(num_repetitions-fails))
...
def main():
    ...
    for num_agents in [100,200,300]:
        ...
        i = 10
        for _ in range(i):
            best_cost, best_span, best_failrate = run_experiment(times=num_experiments, rule_order=best_rule_perm, startpos=startpos, targetpos=targetpos, environment=env, agents_amt=num_agents)
            default_cost, default_span, default_failrate = run_experiment(times=num_experiments, rule_order=[0,1,2,3,4,5,6], startpos=startpos, targetpos=targetpos, environment=env, agents_amt=num_agents)
